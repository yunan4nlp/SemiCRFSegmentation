...
public:
	...
	vector<SparseNode> sparse_output;
	...
	vector<LinearNode> neural_output;
	vector<PAddNode> output;
...
public:
	...
	inline void createNodes(int sent_length, int typeNum){
		...
		neural_output.resize(sent_length);
		sparse_output.resize(sent_length);
...
public:
	inline void initial(ModelParams& model, HyperParams& opts, AlignedMemoryPool* mem){
		...
		for (int idx = 0; idx < maxsize; idx++) {
			...
			neural_output[idx].setParam(&model.olayer_linear);
			sparse_output[idx].setParam(&model.sparse_project);
		}
		...
		for (int idx = 0; idx < maxsize; idx++){
			...
			neural_output[idx].init(opts.labelSize, -1, mem);
			sparse_output[idx].init(opts.labelSize, -1, mem);
...
public:
	...
	inline void forward(const vector<Feature>& features, bool bTrain = false){
		...
		for (int idx = 0; idx < seq_size; idx++) {
			...
			sparse_output[idx].forward(this, feature.linear_features);
		}
		...
		for (int idx = 0; idx < seq_size; idx++) {
			...
			neural_output[idx].forward(this, &(word_hidden3[idx]));
			output[idx].forward(this, &sparse_output[idx], &neural_output[idx]);
		}
	}
